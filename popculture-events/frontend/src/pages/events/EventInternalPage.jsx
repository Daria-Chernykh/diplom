...
import { getUserEventRegistration } from "../../api/registrationsApi.js";
...
function getRegistrationStatusText(registration) {
  if (!registration) {
    return "";
  }

  if (registration.status === "pending") {
    return "Заявка отправлена. После подтверждения организатором вы будете зарегистрированы на мероприятие.";
  }
  ...
  if (registration.status === "rejected") {
    return "Заявка отклонена организатором";
  }
...
export function EventInternalPage() {
  ...
  async function loadEvent() {
    ...
    try {
      ...
      if (isAuthenticated) {
        try {
          const registrationResponse = await getUserEventRegistration(eventId);
  ...
  function handleRegistrationClick() {
    ...
    navigate(`/events/${event.id}/registration`);
  }
  ...
  const registrationStatusText = getRegistrationStatusText(registration);
        ...
        <aside className="card">
          ...
          <div className="info-list">
            ...
            <div className="info-row">
              ...
              <span>Внутренняя регистрация на сайте</span>
            </div>
          ...
          {registrationStatusText && (
            <StatusBadge type={registration?.status === "registered" ? "success" : "warning"}>
              {registrationStatusText}
            </StatusBadge>
          )}

          <div className="form-actions">
            {!registration && !isOwnEvent && (
              <button className="btn btn-primary" type="button" onClick={handleRegistrationClick}>
                Зарегистрироваться
              </button>
            )}
