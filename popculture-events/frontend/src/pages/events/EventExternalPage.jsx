...
import { confirmExternalRegistration } from "../../api/registrationsApi.js";
...
export function EventExternalPage() {
  ...
  async function handleExternalRegistrationConfirm() {
    if (!isAuthenticated) {
      navigate("/login");
      return;
    }
    ...
    try {
      await confirmExternalRegistration(event.id);
      setMessage("Внешняя регистрация подтверждена на платформе.");
    } catch (requestError) {
      setError(requestError.message || "Не удалось подтвердить внешнюю регистрацию.");
    } finally {
        ...
        <aside className="card">
          ...
          <div className="info-list">
            ...
            <div className="info-row">
              ...
              <span>Внешняя регистрация на стороннем сайте</span>
            </div>
          ...
          <div className="note">
            Регистрация выполняется на стороннем ресурсе. Оплата на платформе PopCulture Events не выполняется.
          </div>

          <div className="form-actions">
            <button className="btn btn-primary" type="button" onClick={handleExternalLinkClick}>
            ...
            {!isOwnEvent && (
              <button
                ...
                onClick={handleExternalRegistrationConfirm}
              >
                {isRegistrationLoading ? "Сохранение..." : "Я зарегистрировался"}
              </button>
            )}
