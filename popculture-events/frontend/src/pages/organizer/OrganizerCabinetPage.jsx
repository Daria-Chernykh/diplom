...
import { uploadOrganizerImage, uploadUserProfileImage } from "../../api/filesApi.js";
import { ImageUploader } from "../../components/files/ImageUploader.jsx";

export function OrganizerCabinetPage() {
  ...
  async function loadProfile() {
    ...
    try {
      ...
      setOrganizerImage(loadedUser.organizer_image || null);

      setProfileForm({
        full_name: loadedUser.full_name || "",
        organization_name: loadedUser.organization_name || "",
        organization_description: loadedUser.organization_description || ""
      });
    } catch (requestError) {
      setError(requestError.message || "Не удалось загрузить профиль организатора.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleProfileSubmit(event) {
    ...
    try {
      ...
      setOrganizerImage(updatedUser.organizer_image || organizerImage);

      setMessage(response.message || "Профиль организатора обновлен.");
    } catch (requestError) {
      setError(requestError.message || "Не удалось обновить профиль организатора.");
    } finally {
  ...
  if (isLoading) {
    return <div className="loading-screen">Загрузка профиля организатора...</div>;
  }
    ...
    return (
      <main className="page">
        <section className="card">
          <h1>Профиль организатора недоступен</h1>
    ...
    <main className="page">
      ...
      <section className="main">
        <aside className="sidebar">
          ...
          <div className="menu">
            <Link className="menu-item active" to="/organizer">
              <div>Профиль</div>
            </Link>
        ...
        <section className="content">
          <div className="section-head">
            <div>
              ...
              <p className="muted">
                Организатор может пользоваться функциями участника и создавать мероприятия.
              </p>
            </div>
          </div>
          ...
          <ImageUploader
            title="Изображение организатора"
            currentFile={organizerImage}
            uploadHandler={uploadOrganizerImage}
            onUploaded={(file) => setOrganizerImage(file)}
            onDeleted={() => setOrganizerImage(null)}
            buttonText="Загрузить изображение организатора"
          />
            ...
            <section className="form-section">
              ...
              <div className="field">
                ...
                <input
                  ...
                  onChange={handleProfileChange}
                  placeholder="Например: PopCulture Events"
                />
              </div>
              ...
              <div className="field">
                ...
                <textarea
                  ...
                  value={profileForm.organization_description}
                  onChange={handleProfileChange}
                  placeholder="Расскажите о деятельности организатора"
                />
              </div>
            </section>
