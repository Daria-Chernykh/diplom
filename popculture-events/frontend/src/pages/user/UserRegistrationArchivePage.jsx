...
import { getMyRegistrationArchive } from "../../api/registrationsApi.js";

function getStatusLabel(status) {
  if (status === "pending") {
    return "Ожидала подтверждения";
  }

  if (status === "registered") {
    return "Регистрация была оформлена";
  }
  ...
  if (status === "canceled") {
    return "Регистрация была отменена";
  }
...
export function UserRegistrationArchivePage() {
  ...
  useEffect(() => {
    async function loadArchive() {
      ...
      try {
        const response = await getMyRegistrationArchive();
        setRegistrations(response.registrations || []);
      } catch (requestError) {
        setError(requestError.message || "Не удалось загрузить архив регистраций.");
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <section className="main">
        <aside className="sidebar">
          ...
          <div className="menu">
            ...
            <Link className="menu-item active" to="/user/registrations/archive">
          ...
          {!isLoading && (
            <div className="event-list">
              {registrations.length === 0 && (
                <div className="card">
                  <p className="muted">Архивных регистраций пока нет.</p>
                </div>
              )}
                ...
                <article className="event-row-card" key={registration.id}>
                  ...
                  <div className="event-row-body">
                    <div className="event-row-top">
                      ...
                      <span className="status">{getStatusLabel(registration.status)}</span>
                    </div>
                    ...
                    <div className="meta">
                      <div className="meta-item">
                        {registration.event.event_datetime
                          ? new Date(registration.event.event_datetime).toLocaleDateString("ru-RU")
                          : "Дата не указана"}
                      </div>
